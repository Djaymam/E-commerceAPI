...
static async ratingProduct(clientId, userRating,productId){
    ...
    try {
        ...
        if (!isRated.rows[0]) {
            ...
            const AVG_RATING = `SELECT AVG(rating)::numeric(10,1) FROM ratings WHERE product_id =${productId}`
            await db.query(`UPDATE products SET rating = (${AVG_RATING}) WHERE id=$1`,[productId])
            await db.query("COMMIT")
